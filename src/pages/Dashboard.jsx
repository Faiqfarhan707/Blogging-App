...
const Dashboard = () => {
  ...
  const postBlog = async (event) => {
    ...
    try {
      ...
      const docId = await sendData(blogData, "blogs");
      console.log("Document ID:", docId);
  
      setData((prevData) => [
        ...prevData,
        { ...blogData, docid: docId },
      ]);
    } catch (error) {
  ...
  return (
    <>
      ...
      <form className='flex flex-col gap-4 justify-center items-center'>
        ...
        <button type='submit' className="btn btn-warning" onClick={postBlog}> {loading ? <span className="loading loading-spinner loading-sm"></span> : 'Publish Blogs'}</button>
      </form>
